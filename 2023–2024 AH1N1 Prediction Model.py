...
BATCH_SIZE = 16
SEED = 42
WINDOW = 4  # Sliding window length
...
def build_multimodal_weighted_model():
    ...
    # Branch feature list before fusion
    branch_features = [x_epi, x_climate, x_social, inp_stage]
    branch_concat = concatenate(branch_features)

    # Learnable branch weights generated by softmax normalization
...
def train_final_and_predict(train_df, full_df, predict_start, predict_end, plot=True):
    ...
    # Merge historical windows to ensure that each test week can be predicted
    ...
    # Estimate prediction intervals using training residuals
    ...
    # Save CSV file
    save_predict_result_csv(y_pred, lower, upper, y_p, dates, f"Multimodal_preds_{predict_start}_{predict_end}.csv")
        ...
        for rect in bar:
            ...
            if np.abs(height) > 0.15:
                ax2.text(rect.get_x() + rect.get_width()/2.0, height, f'{height:.2f}',
                         ...
                         fontsize=8, color='#333', fontproperties=myfont)
        plt.subplots_adjust(hspace=0.22)
        plt.show()

    # Save model and interpretation inputs
    model.save('multimodal_model_2023_2024.h5')
    np.savez('multimodal_inputs_2023_2024.npz',
             X_epi_p=X_epi_p,
...
if __name__ == "__main__":
    ...
    if full_df.empty:
        ...
        sys.exit()
    print("Forecasting the 2023-2024 epidemic season (excluding anomalous epidemic seasons)...")
