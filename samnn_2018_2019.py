...
def build_features(df):
    ...
    # Check that the lag columns required for the selected horizon are available
    for col in BASE_EPIDEMIOLOGY:
        if col not in df.columns:
            raise ValueError(f"The input data file is missing the required lag column: {col}")
...
def train_final_and_predict(train_df, full_df, predict_start, predict_end, plot=True):
    ...
    save_predict_result_csv(
        y_pred, lower, upper, y_p, df_predict["date"],
    ...
    model.save(f'revised_multimodal_model_2018_2019_lead{FORECAST_LEAD}wk.h5')
    np.savez(
        f'revised_multimodal_inputs_2018_2019_lead{FORECAST_LEAD}wk.npz',
        X_epi_p=X_epi_p,
...
if __name__ == "__main__":
    ...
    if full_df.empty:
        ...
        sys.exit()
    print("Forecasting the 2018/19 season after excluding low-activity seasons...")
