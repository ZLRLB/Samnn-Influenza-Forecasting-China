...
N_EPI_INPUTS = 3    # Number of epidemiological lag inputs used by this implementation
...
def get_epi_cols_for_lead(df, lead, n_epi_inputs=3):
    """
...
period_params = [
    ...
    {"train_start": "2011-01-05", "train_end": "2024-05-01", "test_start": "2024-10-02", "test_end": "2025-04-02", "name": "2024-2025_scenario"}
]
...
for params in period_params:
    ...
    model = XGBRegressor(n_estimators=150, learning_rate=0.01, random_state=RANDOM_STATE, n_jobs=1)
    model.fit(X_train, y_train)
    pred = model.predict(X_test)

    train_pred = model.predict(X_train)
    stderr = (y_train - train_pred).std(ddof=1)
    ci95 = 1.96 * stderr
    lower = pred - ci95
    upper = pred + ci95
    ...
    pred_df = pd.DataFrame({
        ...
        "y_pred": pred,
        "XGBoost_pred_lower": lower,
        "XGBoost_pred_upper": upper
    })
    pred_df.to_csv(f"revised_XGBoost_preds_{params['name']}.csv", index=False, encoding="utf-8-sig")
    ...
    print(f"\nXGBoost {params['name']}: {params['test_start']} ~ {params['test_end']}")
    print('  R2:', r2)
    print('  MAE:', mae)
    print('  RMSE:', rmse)
    ...
    ax1.plot(x_dates, pred, label="XGBoost prediction", marker='x', color="#D55E00", linewidth=2)
    ...
    ax1.set_title(f"Prediction interval {params['test_start']} - {params['test_end']}: observed and XGBoost predictions with uncertainty interval", fontproperties=myfont)
