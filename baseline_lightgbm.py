...
for params in period_params:
    ...
    if params['test_start'] == "2023-10-04" and params['test_end'] == "2024-04-03":
        print(f"[2023/24 period] 95%Mean bounds of the residual-based uncertainty interval: [{lower.mean():.4f}, {upper.mean():.4f}]")
        ...
        ax1.plot(x_dates, pred, label="LightGBM prediction", marker='x', color="#D55E00", linewidth=2)
        ...
        ax1.set_title(f"Prediction interval {params['test_start']} - {params['test_end']} Observed and LightGBM predictions with uncertainty interval", fontproperties=myfont)
    ...
    pred_df = pd.DataFrame({
        ...
        "y_pred": pred,
        "LightGBM_pred_lower": lower,
        "LightGBM_pred_upper": upper
    })
    pred_df.to_csv(f"revised_LightGBM_preds_{params['name']}.csv", index=False, encoding="utf-8-sig")
    ...
    print(f"\nLightGBM {params['name']}: {params['test_start']} ~ {params['test_end']}")
    print('  R2:', r2)
    print('  MAE:', mae)
    print('  RMSE:', rmse)
