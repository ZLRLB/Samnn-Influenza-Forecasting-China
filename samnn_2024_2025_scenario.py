...
def plot_forecast_with_ci(dates, y_pred, lower, upper, target_name, fontproperties, save_path=None):
    ...
    plt.fill_between(
        ...
        color="#0072B2",
        alpha=0.18,
        label="95% residual-based uncertainty interval",
        zorder=2
    )
    ...
    plt.title("2024/25 A/H1N1 scenario forecast with a 95% residual-based uncertainty interval", fontproperties=fontproperties, fontsize=18, pad=15)
    ax = plt.gca()
...
def train_final_and_predict(train_df, full_df, predict_start, predict_end, plot=True):
    ...
    if len(df_predict) == 0 or len(y_pred_selected) == 0:
        print("No data are available for the requested interval; the plot cannot be generated.")
        return None, None, None, None

    print("\nWeekly 2024/25 scenario predictions and residual-based 95% uncertainty intervals:")
    for dt, pred, l, u in zip(df_predict["date"], y_pred_selected, lower_selected, upper_selected):
        print(f"{dt.strftime('%Y-%m-%d')}: Predicted value={pred:.4f}，lower={l:.4f}，upper={u:.4f}")

    print("\nMean interval bounds: lower=%.4f, upper=%.4f" % (lower_selected.mean(), upper_selected.mean()))
    print("Mean prediction: %.4f" % y_pred_selected.mean())

    # ==== Save unsmoothed predictions to CSV ====
    ...
    print("\nUnsmoothed predictions were saved to forecast_2024-2025_raw.csv")

    # ==== Apply a three-week rolling mean to predictions and interval bounds ====
    window_size = 3
    ...
    if plot:
        plot_forecast_with_ci(
            df_predict["date"], y_pred_smoothed, lower_smoothed, upper_smoothed,
            TARGET, myfont, save_path="forecast_2024-2025.png"
        )
    return y_pred_smoothed, lower_smoothed, upper_smoothed, df_predict["date"]

if __name__ == "__main__":
    print("Loading training data after excluding low-activity seasons and applying the analysis cutoff of 2024-05-01...")
    ...
    print("Loading the 2024/25 scenario feature data...")
    full_df = load_full_data()
    if full_df.empty:
        print("The scenario dataset is empty; predictions cannot be generated.")
        sys.exit()
    print("Generating the 2024/25 scenario forecast...")
    y_pred, lower, upper, pred_dates = train_final_and_predict(train_df, full_df, PREDICT_START, PREDICT_END, plot=True)